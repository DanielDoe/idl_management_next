...
export default () => {
  ...
  const addAllocationElements = allocation => {
    ...
    manageCourseAllocations({
      ...
      notification["success"]({
        ...
        description: `Courses added to allocation successfully.`,
      });
    });
  };
  ...
  const removeAllocationElements = allocation => {
    confirm({
      ...
      content: `If you proceed, the selected allocation data will be deleted`,
      okText: "Yes",
      okType: "danger",
      cancelText: "No",
      onOk() {
        ...
        const newState = allocations.filter(
          element => element.programme_id !== allocation.programme_id
        );
