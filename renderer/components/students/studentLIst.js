...
export default props => {
  ...
  const onSearch = e => {
    ...
    const newData = dataSource.filter(
      s => s.student_title.toLowerCase().search(value) !== -1
    );
  ...
  const renderDetails = data => {
    return (
      <Row gutter={16}>
        <Col span={12}>
          <p style={{ textAlign: "-webkit-auto" }}>Email: {data.email}</p>
        </Col>
        <Col span={12}>
          <p style={{ textAlign: "-webkit-auto" }}>Programme: {data.programme}</p>
        </Col>
      </Row>
    );
  };
      ...
      render: (text, record) => (
        <div className="action-column grid">
          ...
          <button
            ...
            onClick={() => context.removeStudentElements(record)}
          >
            Delete
          </button>
        </div>
